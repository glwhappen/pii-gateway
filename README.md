...
同一真实值（同一手机号/身份证）跨请求复用**同一个占位符**，不管对话多少次都一样，方便多轮上下文连贯。映射存在进程内存的全局 store（`real <-> placeholder`），带锁并发安全；每请求仍把用到的映射复制到请求级 `mapping` 用于还原。

- 同一内容 → 同一占位符；不同内容 → 不同占位符
- ⚠️ **进程内存级**：网关重启后映射清空，同一内容会重新分配占位符。如需跨重启持久（映射落盘），涉及 PII 存储，需另行开启（见局限一节）。
...
- **确定性映射为进程内存级**：网关重启后 `real<->占位符` 映射清空，同一内容会重新分配占位符。若需跨重启保持同一映射（如长期连续会话），需把映射持久化落盘——但这会额外存储 PII 对应关系，属于安全权衡，默认不开启。
