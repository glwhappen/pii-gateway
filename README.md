...
| `PII_CONFIG_FILE` | `config.json` | 运行配置落盘文件（转发目标/端口等，管理面板可改） |
...
- **⚙️ 设置**：转发目标可在线修改（**热生效**，不用重启）；代理端口/管理端口可改（保存后需重启生效）；映射/规则文件路径
- **脱敏自测**：输入一段含 PII 的文本，离线演示「脱敏 → 还原」往返（不调模型）
- **🧩 正则规则**：脱敏匹配规则列表，可增删、落盘持久
...
脱敏匹配规则可在管理面板增删，落盘持久化（`PII_RULES_FILE`）。默认规则（按执行顺序）：

| 规则名 | 正则 | 示例 |
|--------|------|------|
| 银行卡号 | `(?:62\d{14,17}|4\d{15}|5[1-5]\d{14})` | 6222021234567890123 |
| 中国大陆身份证 | `(?i)[0-9]{17}[0-9X]` | 110101199003071234 |
| 中国大陆手机号 | `1[3-9][0-9]{9}` | 13812345678 |
| 邮箱 | `[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}` | abc@test.com |
| 中国大陆车牌 | `[京津沪渝...使领][A-HJ-NP-Z][A-HJ-NP-Z0-9]{5,6}` | 粤B12345 |
| 固定电话 | `0\d{2,3}-?\d{7,8}` | 0755-12345678 |
| 中国护照 | `(?i)[eghpsd]\d{8}` | E12345678 |
